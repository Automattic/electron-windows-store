...
function signAppx (program) {
  return new Promise((resolve, reject) => {
    ...
    } else if (program.devCert.toLowerCase() === 'nil') {
      utils.debug('*** Nil developer certificate, no signing ***')
      return resolve('unsigned')
    }

    const pfxFile = program.devCert
    const appxFile = path.join(program.outputDirectory, `${program.packageName} ${program.packageVersion}.appx`)
